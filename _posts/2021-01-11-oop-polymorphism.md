...
또한 다형성의 고급 스킬이라 할 수 있는 디자인 패턴에 대해서도 배워 볼 수 있습니다.
...
```
유튜브와 같은 동영상 플랫폼을 개발하려고한다. 사용자 메뉴는 인기영상, 최신영상, 구독영상으로 총 3개로 구분되어있으며 각 목록 페이지에 페이징 대신 하단에 더보기 버튼을 만들어 더보기 버튼 클릭 시, 추가 데이터를 불러오게 하고싶다.
```
...
public class SeeMoreService {
    ...
    // 인기영상 HotVideoService 구현체를 의존 하고있다.
...
만약에 나중에 고객이 좋아요영상, 싫어요영상, 나중에볼영상, 이달의 영상 등 여러 요구사항(목록 페이지)가 추가될 때마다 마냥 위처럼 코드를 늘릴순 없습니다.
...
위에서 계속 반복되서 나오는 다형성의 특징인 `역할에 의존해야지 구현에 의존하면 안된다`에서 역할은 인터페이스를 의미한다는 것을 배웠습니다. 이렇듯 다형성과 인터페이스는 거의 한 몸, 자웅동체라고 생각하면 됩니다.
...
즉, 같은 이름의 print 메서드라도 서도 다르게 응답하고 있으며, 다형성을 구현하는 방법 중 하나입니다.
...
이처럼 같은 drive 메서드라도 서도 다르게 응답하고 있으며, 다형성을 구현하는 방법 중 하나입니다.
